...
final_data_summedoverbins %>%
  dplyr::filter(fish_id %in% c(seq(10,15))) %>%
...
Cleal et al. showed that zebrafish naturally perform more of the alternation tetragrams (LRLR and RLRL) in a Y-maze. This is also obsevred in our data here. A few fish may be outliers, as they have performed abnormally high number of tetragrams (labelled plots below). 
...
final_data_summedoverbins %>%
  ...
  ylab("Total number of turns") +
  ggtitle("Total number of turns performed by fish in the Y-maze in 1 hour", 
          subtitle = "Fish in Tank 3 appear to be more active")
```
...
final_data %>% 
  ...
  geom_label_repel(aes(label = paste0("fish " , fish_id)), 
                   fill = NA,
                   nudge_x = -0.5, 
                   data = . %>% dplyr::filter(total_turns > 150)) +
  geom_quasirandom(aes(shape = Tank)) +
  scale_fill_viridis_d(option = "viridis") +
  ylab("Number of turns") +
  theme(legend.position = "top") +
  ggtitle("Number of turns performed by fish in the Y-maze in 1 hour")
```
