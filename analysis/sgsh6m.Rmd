...
meta <-
  read_excel("data/metadata/sgsh 6 month behavioural metadata.xlsx") %>%
  mutate(
...
final_data <- read_csv("data/processed_data/sgsh/6m/final_output.csv") %>%
  dplyr::select(-1) %>%
...
The plots below show the raw counts of each of the possible tetragrams. We can see some fish were quite inactive, wih low total number of tetragrams. The plot below show fish 1-3 for vis purposes. To see all fish, remove the `dplyr::filter` line of code.

```{r fig.height = 20, fig.width = 20}
final_data_summedoverbins %>%
  dplyr::filter(fish_id %in% c(seq(1,3))) %>%
...
We can also overlay the genotype boxplots as shown below. Looks like the heterozyous and homozygous fish perform more alternations than the WT fish. However, I will look into this more later in the analysis.
...
```{r}
read_csv("data/processed_data/sgsh/6m/time_in_zone.csv") %>% 
  mutate(fish_id = as.character(fish_id)) %>%
  dplyr::select(fish_id, bin, zone, time_in_zone) %>%
  left_join(meta) %>%
  dplyr::filter(Keep == "Y") %>%
  dplyr::filter(zone %in% seq(1,4)) %>%
...
print(emmeans(glm, specs = "bin", by = "Genotype"), type = "response") %>%
  ...
  ggtitle("GLM predicted probability of zebrafish performing an alternation\ntetragram across the 6x10 min bins",
          subtitle = paste0("Effect of bin:Genotype p = ",
...
No statistically significant differences are observed for *sgsh* ^I388*^ mutant zebrafish for alterations to spatial working memory at 6m of age. 
